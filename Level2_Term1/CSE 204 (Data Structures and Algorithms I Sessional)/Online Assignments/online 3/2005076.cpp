...
public:
    ...
    int nodeHeight(node<data_type> *rootNode)
    {
        int leftHeight;
        int rightHeight;
        ...
        leftHeight = nodeHeight(rootNode->left);
        rightHeight = nodeHeight(rootNode->right);

        return leftHeight > rightHeight ? 1+leftHeight : 1+rightHeight;

    }

    bool for_balanced(node<data_type> *rootNode)
    {
        int leftHeight;
        int rightHeight;
        ...
        if(abs(leftHeight-rightHeight) <=1 && for_balanced(rootNode->left) && for_balanced(rootNode->right)){
            return true;
        }
        else
            return false;
