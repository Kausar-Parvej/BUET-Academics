...
int getCapacity(vector<pairs> G, int p){
    for(auto temp : G){
        if(temp.first == p)
            return temp.second;
    }
}
void setCapacity(vector<pairs> G, int p, int value){
    for(auto temp : G){
        if(temp.first == p)
            temp.second = value;
            return;
...
bool bfs(vector< vector<pairs> > r_adj, int n, int s, int t){
    bool visit[n] = {false};
    ...
    while(!q.empty()){
        ...
        for(auto v : r_adj[u]){
            if(visit[v.first] == false && v.second > 0){
                if(v.first == t){
...
bool max_flow(int team){
    ...
    for(int i=0; i<adj.size(); i++){
        ...
        for(auto temp : adj[i]){
            rGraph[i].push_back(temp);
            rGraph[temp.first].push_back(make_pair(i,0));
    ...
    while(bfs(rGraph,num,s,t)){
        for(int v = t; v != s; v = bfs_parent[v]){
            int u = bfs_parent[v];
            minn = getCapacity(rGraph[u],v) < minn ? getCapacity(rGraph[u],v) : minn;
        }
        for(int v = t; v != s; v = bfs_parent[v]){
            int u = bfs_parent[v];
            setCapacity(rGraph[u],v,getCapacity(rGraph[u],v)-minn);
            setCapacity(rGraph[v],u,getCapacity(rGraph[v],u)+minn);
        }
    }

    for(auto temp : rGraph[s]){
        // cout<<flow[s][temp]<<" ";
        if(temp.second > 0) return true;
    }
    return false;
