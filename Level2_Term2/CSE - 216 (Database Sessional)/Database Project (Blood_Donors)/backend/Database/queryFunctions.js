...
export async function findPerson(P_ID){
    let query = select_table('\"PERSON\"');
    const binds = {};
...
export async function find_user(user){
    let query = select_table('\"USER\"');
    const binds = {};
...
export async function find_admin(admin){
    let query = select_table('\"ADMIN\"');
    const binds = {};
...
export async function getUserRatedBooksDB(context){
    let query = select_table("BOOK B JOIN RATING R ON(B.ISBN=R.ISBN)");
...
export async function getUserReviewedBooksDB(context){
    let query = select_table("BOOK B JOIN REVIEW R ON(B.ISBN=R.ISBN)");
    query+="\nWHERE R.PERSON_ID = :PERSON_ID";
...
export async function getAllAwardsDB(){
  ...
  export async function getBookshelvesDB(bookshelves) {
    ...
    let query = select_table("BOOKSHELF");
    query += "\nWhere PERSON_ID = :PERSON_ID";
...
export async function getTopBookDB(context){
    ...
    let query = select_table("(" + query01 + ")");
...
export async function getRecentBookDB(context){
    let query = select_table("(SELECT ISBN, TITLE, COVER_IMAGE, PUBLICATION_DATE FROM BOOK ORDER BY PUBLICATION_DATE DESC)");
...
export async function getGenreBookDB(context){
    let query = select_table("BOOK B join BOOK_GENRE BG ON(B.ISBN = BG.ISBN) join GENRE G ON(BG.GENRE_ID = G.GENRE_ID)");
    const binds = {};
