...
import { find_admin, findPerson, find_user, addAdmin, addUser } from '../Database/queryFunctions.js';
export async function loginUser(req, res, next) {
    try {
      ...
      const foundUser = await find_user(user);
  ...
  export async function loginAdmin(req, res, next) {
    try {
      ...
      const foundAdmin = await find_admin(admin);
  ...
  export async function decodeToken(req, res){
    try {
        ...
        if(decoded.ROLE==='user'){
            ...
            const foundPerson = await findPerson(foundUser[0].PERSON_ID);
            ...
            const foundPerson = await findPerson(foundAdmin[0].PERSON_ID);
