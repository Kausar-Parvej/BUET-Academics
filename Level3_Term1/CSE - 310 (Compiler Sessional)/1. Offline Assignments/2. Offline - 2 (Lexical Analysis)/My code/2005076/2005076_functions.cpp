...
int symbolTable_size = 10;



ofstream token_out("2005076_token.txt");
ofstream log_out("2005076_log.txt");

SymbolTable symbol_table(symbolTable_size);
...
void identifier_token(string id){
    ...
    if(symbol_table.currentScope->lookUp(id) == nullptr){
        ...
        log_out<<symbol_table.print_allScope();
    }
    else
