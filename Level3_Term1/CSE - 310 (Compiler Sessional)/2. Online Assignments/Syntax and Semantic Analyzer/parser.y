...
%token <symbol_info> IF ELSE FOR LOOP WHILE DO BREAK RETURN SWITCH CASE DEFAULT CONTINUE PRINTLN ADDOP INCOP DECOP RELOP ASSIGNOP LOGICOP BITOP NOT LPAREN RPAREN LCURL RCURL LSQUARE RSQUARE COMMA SEMICOLON INT CHAR FLOAT DOUBLE VOID
...
statement : var_declaration {
	...
	|LOOP lcurls statement RCURL WHILE LPAREN expression RPAREN SEMICOLON{
		print_grammar_rule("statement", "LOOP LCURL statement RCURL WHILE LPAREN expression RPAREN SEMICOLON");
		$$ = new SymbolInfo("", "statement");
		$$->set_rule("statement : LOOP LCURL statement RCURL WHILE LPAREN expression RPAREN SEMICOLON");
		$$->add_child($1); $$->add_child($2); $$->add_child($3); $$->add_child($4); $$->add_child($5);
		$$->add_child($6); $$->add_child($7); $$->add_child($8); $$->add_child($9);
