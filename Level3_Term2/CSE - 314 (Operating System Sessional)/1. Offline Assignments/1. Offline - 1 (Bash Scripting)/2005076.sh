...
do  
    ...
    if [[ $isArchived == "true" ]]; then                                   # [[ ]] -> for advanced string comparison
        if [ -d $submission ]; then
            ...
            case $archiveExtension in
                "zip") 
                    unzip -n -q  $submission -d $submissionDirectory
                    ;;

                "tar")
                    mkdir -p "$submissionDirectory"
                    tar -xf $submission -C $submissionDirectory
                    ;;

                "rar") 
                    mkdir -p "$submissionDirectory"
                    unrar x -inul $submission $submissionDirectory
                ;;
            esac
        fi
    ...
    do
        ...
        if [[ -f "$submissionDirectory/$sId/$sId.$language" ]]; then
            validLanguage=true
            fileLang=$language
            submittedFile="$submissionDirectory/$sId/$sId.$language"
            #echo $submittedFile
            break
        fi
    done
    ...
    #echo $submittedFile
    case $fileLang in
        c) gcc "$submittedFile" -o "$submissionDirectory/$sId/$sId" && "$submissionDirectory/$sId/$sId" > "$outPutFile" ;;
        cpp) g++ "$submittedFile" -o "$submissionDirectory/$sId/$sId" && "$submissionDirectory/$sId/$sId" > "$outPutFile" ;;
