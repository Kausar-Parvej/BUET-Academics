...
RoutingProtocol::RecvRaodv(Ptr<Socket> socket)
{
    ...
    case AODVTYPE_REV_RREQ: {
        RecvReply_reverse(packet, receiver, sender);
        break;
    }
    case AODVTYPE_RERR: {
...
RoutingProtocol::SendReply(const RreqHeader& rreqHeader, const RoutingTableEntry& toOrigin)
{
    ...
    //Handle the sequence number logic for the destination node
    if (!rreqHeader.GetUnknownSeqno() && (rreqHeader.GetDstSeqno() == m_seqNo + 1))
    {
        m_seqNo++;
    }

    //Create the RREP header
    RrepHeader rrepHeader(/*prefixSize=*/0,
                          /*hopCount=*/0,
                          /*dst=*/rreqHeader.GetDst(),
                          /*dstSeqNo=*/m_seqNo,
    ...
    //Create the AODVTYPE_REV_RREQ packet
    Ptr<Packet> packet = Create<Packet>();
    packet->AddHeader(rrepHeader);
    TypeHeader tHeader(AODVTYPE_REV_RREQ);  // Ensure RREP type
    packet->AddHeader(tHeader);

    //RAODV modification: Broadcast the RREP to neighbors
    for (auto i = m_socketAddresses.begin(); i != m_socketAddresses.end(); ++i)
    {
        Ptr<Socket> socket = i->first;
        Ipv4InterfaceAddress iface = i->second;

        // Ensure the broadcast is sent from the correct interface
        if (iface.GetLocal() == toOrigin.GetInterface().GetLocal())
        {
            // Broadcast the packet
            socket->SendTo(packet->Copy(), 0, InetSocketAddress(Ipv4Address::GetBroadcast(), AODV_PORT));
        }
    }

    NS_LOG_DEBUG("Broadcasted RREP from destination " << rreqHeader.GetDst());
}
...
void
RoutingProtocol::RecvReply_reverse(Ptr<Packet> packet, Ipv4Address sender, Ipv4Address receiver)
{
    NS_LOG_FUNCTION(this << "Received reverse RREP from " << sender);

    RrepHeader rrepHeader;
    TypeHeader typeHeader;

    //Remove headers and validate the packet type
    packet->RemoveHeader(typeHeader);
    packet->RemoveHeader(rrepHeader);

    if (!typeHeader.IsValid())
    {
        NS_LOG_WARN("Invalid reverse RREP received, dropping.");
        return;
    }

    //Extract destination and origin information
    Ipv4Address destination = rrepHeader.GetDst();
    Ipv4Address origin = rrepHeader.GetOrigin();

    NS_LOG_DEBUG("Reverse RREP for destination: " << destination << ", origin: " << origin);

    //Update the routing table with the reverse route
    Ptr<NetDevice> dev = m_ipv4->GetNetDevice(m_ipv4->GetInterfaceForAddress(receiver));
    RoutingTableEntry newEntry(
        /*dev=*/dev,
        /*dst=*/destination,
        ...
        /*seqNo=*/rrepHeader.GetDstSeqno(),
        /*iface=*/m_ipv4->GetAddress(m_ipv4->GetInterfaceForAddress(receiver), 0),
        /*hops=*/rrepHeader.GetHopCount() + 1,
        /*nextHop=*/sender,
        /*lifetime=*/rrepHeader.GetLifeTime());

    RoutingTableEntry existingEntry;
    if (m_routingTable.LookupRoute(destination, existingEntry))
    {
        uint8_t currentHopCount = existingEntry.GetHop(); // Use GetHop()
        if (
            !existingEntry.GetValidSeqNo() ||
            (rrepHeader.GetDstSeqno() > existingEntry.GetSeqNo()) ||
            (rrepHeader.GetDstSeqno() == existingEntry.GetSeqNo() &&
             rrepHeader.GetHopCount() + 1 < currentHopCount))
        {
            m_routingTable.Update(newEntry);
        }
    }
    ...
    {
        // Add the reverse route if it doesn't exist
        m_routingTable.AddRoute(newEntry);
    }

    //Forward the reverse RREP to neighbors
    if (rrepHeader.GetHopCount() + 1 < MAX_HOP_COUNT) // Define MAX_HOP_COUNT as a constant
    {
        rrepHeader.SetHopCount(rrepHeader.GetHopCount() + 1);

        Ptr<Packet> forwardPacket = Create<Packet>();
        forwardPacket->AddHeader(rrepHeader);
        forwardPacket->AddHeader(typeHeader);

        for (const auto& socketIface : m_socketAddresses)
        {
            Ptr<Socket> socket = socketIface.first;
            Ipv4InterfaceAddress iface = socketIface.second;

            // Forward the packet as a broadcast
            socket->SendTo(forwardPacket->Copy(), 0, InetSocketAddress(Ipv4Address::GetBroadcast(), AODV_PORT));
        }
    }
}
