...
# Configuration
DATASET_PATH = "dataset.json"   #dataset_70.json
...
class FingerprintDataset(Dataset):
    def __init__(self, json_path, input_size):
        with open(json_path, 'r') as f:
            raw_data = json.load(f)

        self.website_to_idx = {
            name: idx for idx, name in enumerate(sorted(set(d["website"] for d in raw_data)))
        }
        self.idx_to_website = {v: k for k, v in self.website_to_idx.items()}
        self.traces = []
        self.labels = []
        ...
        for entry in raw_data:
            ...
            if len(trace) < input_size:
                trace += [0] * (input_size - len(trace))
            self.traces.append(trace)
            self.labels.append(self.website_to_idx[entry["website"]])
...
def main():
    ...
    # Load dataset
    dataset = FingerprintDataset(DATASET_PATH, INPUT_SIZE)
    ...
    # Choose model
    num_classes = len(dataset.website_to_idx)
    ...
    print(f"Training model on {len(train_dataset)} traces, testing on {len(test_dataset)} traces")
    best_acc = train(model, train_loader, test_loader, criterion, optimizer, EPOCHS, os.path.join(MODELS_DIR, "best_model.pth"))
    

    # Evaluate
    evaluate(model, test_loader, [dataset.idx_to_website[i] for i in range(num_classes)])
