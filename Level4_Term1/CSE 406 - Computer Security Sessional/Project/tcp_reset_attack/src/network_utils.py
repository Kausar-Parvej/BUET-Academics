...
class NetworkUtils:
    ...
    def is_youtube_traffic(packet):
        """Check if packet is YouTube traffic"""
        ...
        # Check for HTTPS traffic (port 443) with large payload
        if (tcp_layer.sport == 443 or tcp_layer.dport == 443) and len(packet) > 1400:
            # Additional checks for video streaming characteristics
            if tcp_layer.flags & 0x18:  # PSH+ACK flags (data packets)
                # Check for YouTube IP ranges (Google's AS15169)
                youtube_ranges = [
                    "172.217.0.0/16",  # Google services
                    "216.58.0.0/16",   # Google services  
                    "74.125.0.0/16",   # Google services
                    "64.233.0.0/16",   # Google services
                    "142.250.0.0/15",  # Additional Google ranges
        ...
        # Check for HTTP video traffic on alternative ports
        if tcp_layer.sport in [80, 8080] or tcp_layer.dport in [80, 8080]:
            if len(packet) > 1400 and tcp_layer.payload:
                # Simple heuristic for video content
                payload_str = str(tcp_layer.payload)
                if any(keyword in payload_str.lower() for keyword in ['video', 'stream', 'youtube', 'googlevideo']):
                    return True
