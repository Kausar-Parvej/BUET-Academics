...
void keyboardListener(unsigned char key, int x, int y)
{
    ...
    case 'a':
        baseRotateAngele -= 4.0f;
        if (baseRotateAngele < 0.0) baseRotateAngele += 360.0;
        break;
        
    case 'd':
        baseRotateAngele += 4.0f;
        if (baseRotateAngele > 360.0) baseRotateAngele -= 360.0;
        break;
...
void dreawTriangle(point p1, point p2, point p3)
{
...
void drawpolygon2D(int n, GLfloat radius)
{
    glBegin(GL_POLYGON);
    for (int i = 0; i < n; i++)
    {
        GLfloat angle = 2 * M_PI * i / n;
...
void drawpolygon3D(int n, GLfloat radius)
{
    point vertices[n][2];
    float height_y = 0.12;
    for (int i = 0; i < n; i++)
    {
        GLfloat angle = 2 * M_PI * i / n;
        vertices[i][0].x = radius * cos(angle);
        vertices[i][0].y = 0.0;
        vertices[i][0].z = radius * sin(angle);

        vertices[i][1].x = radius * cos(angle);
        vertices[i][1].y = height_y;
        vertices[i][1].z = radius * sin(angle);
    }

    for (int i=0; i<n; i++){
        if(i%2==0) glColor3f(1.0f, 1.0f, 0.0f);
        else glColor3f(0.0f, 1.0f, 1.0f);

        glBegin(GL_QUADS);
        glVertex3f(vertices[i][0].x, vertices[i][0].y, vertices[i][0].z);
        glVertex3f(vertices[(i+1)%n][0].x, vertices[(i+1)%n][0].y, vertices[(i+1)%n][0].z);
        glVertex3f(vertices[(i+1)%n][1].x, vertices[(i+1)%n][1].y, vertices[(i+1)%n][1].z);
...
void drawPole(point p1, point p2, point p3, point p4, point p5, point p6, point p7, point p8)
{
    ...
    drawFace(p1,p2,p6,p5);
    drawFace(p4,p3,p7,p8);
    
}

void drawBlade(GLfloat radius){
    dreawTriangle({0.0, 0.0, 0.0}, {0.0, 0.05, radius}, {0.0, -0.05, radius});
}

void drawFanBlade(GLfloat radius)
{
    drawRectangle({radius, 0.0, 0.004}, {radius, 0.0, -0.004},{-radius, 0.0, -0.004}, {-radius, 0.0, 0.004});
}

// 3 triangular blades at 120° apart
void drawFan(GLfloat radius)
{
    glPushMatrix();
    glColor3f(1.0f, 0.0f, 0.0f); // red
    drawFanBlade(radius);
...
void display()
{
    ...
    glRotatef(baseRotateAngele, 1.0f, 0.0f, 0.0f); // Rotate the base around the Y-axis


    //drawpolygon2D(8, 0.2);
    drawpolygon3D(8, 0.2);  //outer body
    drawpolygon3D(8, 0.02);  //inner rod

    glTranslatef(0, 0.12, 0);
    glRotatef(rotationAngle, 0.0f, 1.0f, 0.0f); // Rotate the fan blades around the Y-axis
    drawFan(0.18);
